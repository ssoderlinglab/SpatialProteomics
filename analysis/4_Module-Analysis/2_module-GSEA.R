...
# Project Directories
datadir <- file.path(root, "newdata")
...
part_entrez <- setNames(partition,
			nm=geneLists::mapIDs(names(partition),"uniprot","entrez"))

print((sapply(gene_lists, function(x) sum(x %in% names(part_entrez))<3)))
...
pbar <- txtProgressBar(min = 0, max = length(gene_lists), style = 3)
...
# save gene lists
myfile <- file.path(root,"newdata","gene_lists.rda")
save(gene_lists, file=myfile,version=2)
message("saved: ", myfile)

# save as rda
module_gsea <- sig_dt
myfile <- file.path(root, "newdata", "module_gsea.rda")
