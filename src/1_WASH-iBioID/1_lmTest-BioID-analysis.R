...
## Output in root/data:
# * wash_inter
actome.rda -- the WASH iBioID proteome (sig enriched prots)
...
tidyProteomicsData <- function(raw_data,id.vars,species=NULL,
...
#	})
	...
	if (!all(idx)) {
		...
		msg <- paste(n_out,"proteins are not from", 
		             "“", species, "”", #lquote(species),
			     "and will be removed.")
...
# project directories:
datadir <- file.path(root,folder) # key datasets
...
workingdir <- getwd()
oneback <- dirname(workingdir)
parent_directory <- dirname(oneback)
myfile <- file.path(parent_directory, "data", tools::file_path_sans_ext(zipfile), datafile)
print(myfile)
...
# tidy-up the data
tidy_prot <- tidyProteomicsData(raw_prot,species="Mus musculus",
...
myfile <- file.path(root,folder,"wash_bioid.rda")
save(wash_bioid, file=myfile, version=2)
message("saved: ", myfile)

# save bioid results as rda
myfile <- file.path(root,folder,"bioid_results.rda")
...
myfile <- file.path(root, folder,"wash_interactome.rda")
